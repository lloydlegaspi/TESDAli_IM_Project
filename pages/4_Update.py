...
st.markdown("<h1 style='color: blue; text-align:center;'>Update a Record</h1>", unsafe_allow_html=True)

# Initialize session state for selected fields and new values
...
with col2:
    ...
    if st.session_state.record is not None and st.session_state.columns is not None:
        ...
        selected_fields = st.multiselect("Select Fields to Update:", st.session_state.columns, default=st.session_state.selected_fields)
        
        # Update session state with the selected fields
        st.session_state.selected_fields = selected_fields

        # Allow the user to enter new values for the selected fields
        for field in selected_fields:
            if field not in st.session_state.new_values:
                st.session_state.new_values[field] = st.session_state.record[st.session_state.columns.index(field)]
            new_value = st.text_input(f"New {field}:", st.session_state.new_values[field], key=field)
            st.session_state.new_values[field] = new_value

        # Update the record if the user clicks the "Update Record" button
        if st.button("Update Record"):
            if selected_fields:
                update_data = {field: st.session_state.new_values[field] for field in selected_fields}
                update_record(table, record_id, update_data)
                st.success(f"Record in {table} table updated successfully")
            else:
